...
└─ src/
   ...
   └─ components/
      ...
      ├─ MonthlyBreakdown.svelte# 今月の費目別 実績/予定（予算）＝**ホームの本体**。区分ごとにカード＋進捗バー＋予定超過の赤＋「予定どおりなら残り」
...
- **予実ビュー（予算を『予定額』として実績で上書き）**: Excelの「各月シート」＋「収支表」の代替。**予算を予定額として使い、実績があれば実績・無ければ予定（薄字）で表示**（＝事前に予定を入れておき支出登録で置き換え）。**月次はホーム**（MonthlyBreakdown）、**年間は「年間」タブ**（YearSummary）に分かれている。
  - **月次（＝ホーム本体・MonthlyBreakdown.svelte）**: 予算月を切替、費目別に **実績／予定（予算）** を区分ごとのカードで表示。予定超過は赤＋進捗バー。ヒーローは「今月の残り（実績）＋予定どおりなら残り」＋収入／支出／予定支出。
