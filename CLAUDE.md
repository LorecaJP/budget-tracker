...
└─ src/
   ...
   └─ components/
      ...
      └─ Settings.svelte        # カテゴリ/口座/予算/定期 + 月の開始日 + 扶養トラッカー設定（時給/上限）+ 貯金の目標 + 口座への配分
```
...
- **設定**（Settings.svelte）: カテゴリ/口座/予算/定期の管理 ＋ **月の開始日**（settings.month_start_day を変更可）＋ **扶養トラッカー設定**（えみの時給・年間上限）＋ **貯金の目標**（貯蓄カテゴリの目標額）＋ **口座への配分**（配分プラン）。
