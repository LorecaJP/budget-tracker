...
└─ src/
   ...
   │  ├─ ics.ts            # iPhone純正カレンダーへの追加（.ics 生成→iOSの「カレンダーに追加」起動）。OAuth/サーバ不要。ShiftCalendar の「この月をカレンダーに追加」で使用
   ...
   └─ components/
      ...
      ├─ FuyouTracker.svelte    # 扶養トラッカー（えみ）。暦年でえみ給料を集計し上限までの残り金額/時間。タブ「扶養」（独立タブのまま）。**シフト→見込みの集計を自分で持つ**（`listShifts`＋`shiftHoursByPay`/`estFor`、見込み合計＝`estimateTotal`＝Σ`estFor`）→ 確定（課税）＋見込みで「あと働ける」を算出。**シフト入力は「📅カレンダーでシフトを入力」ボタンから `ShiftCalendar` を直接開く**（変更時は `reloadShifts` でシフトのみ再取得＝モーダルを閉じない）。**見込みの唯一の表示/編集場所は『月別のえみ給料』リスト**：未受給月をタップ→時間(h/m)入力で手動登録（×時給→`fuyou_overrides`、就労月=支給月-1）／手動上書きが無ければシフト由来（就労月の総時間×時給）を「見込み（シフト ○h）」として自動表示。集計は支給日ベースの暦年なので「今年あと」＝今年12月支給ぶんまで（12月の就労ぶんは翌1月支給＝翌年カウント）。**旧 ShiftPlanner.svelte は撤去**（同じ見込みが2リスト＋2エディタに分かれて操作が複雑だったため、本コンポーネントに一本化）
      ├─ ShiftCalendar.svelte   # シフト入力の月カレンダー（ボトムシート）。日付タップで1日1シフトを登録/編集/削除。**よく使うシフトの1タップ登録**（13:00〜20:30 / 9:30〜17:00 のクイックボタン＝`quick()`が開始/終了をセットして即保存）。休憩は「労働(開始〜終了)が6時間超のとき自動90分／6時間ちょうど以下は休憩なし」＝breakFor()で算出（手入力なし）。1画面に収まる高さ＋背景スクロール固定（body overflow:hidden ＋ .sheet overscroll-behavior:contain）。grid は minmax(0,1fr)＋セルは固定高さ（aspect-ratio はこの環境の grid で崩れる）。空セルは淡色で枠を均一に。**開始/終了は横並び**（各 flex:1・中央寄せ＋左右padding詰めで重ならない）。**時刻は15分刻み**(`step="900"`＝iOSの分ホイールが15分単位)。**削除は編集ヘッダーの小リンク**（保存ボタン横には置かない＝保存はフル幅）
...
- **扶養トラッカー**（FuyouTracker.svelte / タブ「扶養」）: えみが夫の扶養を外れないよう、**暦年(1〜12月)・支給日ベース**で `category='えみ給料'` の収入を集計。**ヒーローカード**（数字/ITが苦手な妻向けにシンプル化）: **信号（緑〜79% / 黄80〜95% / 赤96%〜）＋メッセージ**（緑「まだまだ大丈夫」/ 黄「そろそろ気をつけてね」/ 赤「もうギリギリ」、超過時のみ「超えちゃった」）。その下に **「今年あと」「1ヶ月あたりあと」** × **[時間 / 金額]** を**固定サイズ62pxの4タイル**（Lucide: clock=時間=緑 / banknote=金額=青。折り返さず常に同サイズ）。進捗バー＋「○万円まで○%」。超過時はタイル0表示＋「上限を○超えています」。`1ヶ月あたり`＝**今年の残り枠 ÷「まだ予定が入っていない月数」**（＝実績も見込みも無い未来の月。`openMonths`。見込みを入れた月はもう残り枠から差し引かれているので分母から外す＝本当に空いている月あたりの目安。例: 6〜8月に見込みを入れたら9〜12月の4ヶ月で割る）。`openMonths===0` は0表示。**「今年あと」の合計は従来どおり 上限−（確定＋見込み）**。明細（課税支給額・総支給・通勤手当・総労働時間・着地見込み）は**別カード**。月別（社保の月8.8万ライン警告つき）も表示。色トークンは `app.css` の `--fy-*`（ダーク対応）、スタイルは `.fy-*`。**上限判定は課税支給額（通勤手当を除く＝103万手当・税の正しい基準。`課税支給額＝総支給−通勤手当` で算出。明細の課税支給額は年内累計なので使わない）**で、**総支給（通勤手当込み）も併記**。さらに `payslip_details` から **通勤手当の合計・総労働時間の合計（週平均h／20時間接近で警告）** も蓄積表示。**年末調整還付（category=null）は報酬でないため集計対象外**。時給・年間上限は `settings.emi_hourly_wage/emi_year_cap`（設定タブで変更可、`db.ts` の `getFuyouConfig/saveFuyouConfig`。列未追加でも既定値で動作）。集計は暦年なので**予算月(25日始まり)とは別軸**。
  - **シフト・見込み（FuyouTracker.svelte に一本化／扶養タブ内）**: えみが**シフト（就労日＋開始/終了/休憩）を `ShiftCalendar` で入力**すると、**就労月ごとの見込み課税収入＝Σ時間×時給**を FuyouTracker が算出（`shiftHoursByPay`/`estFor`。`fuyou_overrides` で**手動上書き可**＝残業・早上がりの調整、上書きは『月別のえみ給料』のタップ編集で）。**支給は就労月の翌月末**なので、pay-month p の見込みは work-month(p-1) のシフトから出し、**実給与が入った月は見込みを0**（`estFor` が `grossM[p-1]>0` で0を返す）にして二重計上しない（`db.ts` の `listShifts/upsertShift/deleteShift/listFuyouOverrides/setFuyouOverride`）。FuyouTracker は **確定（課税）＋見込み（=`estimateTotal`）** をヒーローの「あと働ける 円/時間」に反映（明細に「確定／見込み／合計」を併記）。**※旧 ShiftPlanner.svelte は撤去**＝見込みの表示/編集は『月別のえみ給料』に一本化（操作の二重化を解消）。これで**毎月10日のシフト提出時に「上限まであと何時間/円 働けるか」**が分かる。**未受給でも金額が分かっている給料（例：5月就労ぶんを6/30受給）を見込みで先に織り込める**。確定シフトは `ShiftCalendar` の「📅 この月をiPhoneカレンダーに追加」(.ics) で iPhone 純正カレンダーへ（→TimeTree は端末カレンダー表示で映る。§7）。`shifts`/`fuyou_overrides` 未作成でも見込み0で動く（SQL: `supabase/shifts.sql`）。
...
4. 確定で **総支給＝収入1件**（カテゴリ: えみ給料/ゆうき給料/賞与は「ボーナス」）＋ **各控除＝tax区分の支出**（健康保険/子育支援金/厚生年金/雇用保険/住民税/所得税。同じ口座から。対応カテゴリが無ければ税区分で自動作成＝`ensureCategory`）を登録。`source='ocr'`。重複（同日同額）警告・差引支給額の不一致警告あり。給料(salary)は併せて **`payslip_details`**（通勤手当・総労働時間。`parse.ts` が `通勤手当` と `総労働時間`(HH:MM→分) を抽出）も保存し、扶養トラッカーの蓄積に使う（レビュー画面で確認・手修正可）。
...
### パーサのポイント（実データで検証済み）
- 控除の内訳は **Azureの表(tables)のセル**から取る（ラベルの真下＝行+1・同列）。本文(content)はラベルと値が離れるため誤対応する。小計（社会保険料計）・課税対象額・控除合計は内訳に含めない。拾う控除は `parseAzure.ts` の `dedDefs`（健康保険/**子育支援金**/厚生年金/雇用保険/住民税/所得税）。**子育支援金＝2026年開始の「子ども・子育て支援金」**（健保と別枠で控除。実害あり・対処済み：これを拾わないと控除合計より368円等ぶん過少になり「差引支給額が一致しません」の誤警告が出る＝ゆうき2026-07明細で発生）。新しい控除項目が明細に増えたら `dedDefs` に追加する。seedの税カテゴリにも `子育支援金` を追加済み。**既存DBで未作成の控除カテゴリは、取込時に `PayslipImport.save()` が `ensureCategory(label, 'tax', uid)` で税区分カテゴリを自動作成して紐づける**（＝「未マッチ：区分なし」を解消。正の控除のみ対象。還付＝マイナス控除は収入扱いで対象外）。テキストPDF側(`parse.ts`＝えみ様式)は控除合計のみを1件で使うため内訳変更の影響なし。
...
### 中（機能拡充）
- **シフト→iPhone純正カレンダー（.ics・実装済み）**: `ShiftCalendar` の **「📅 この月をiPhoneカレンダーに追加」** ボタンで、その月のシフトを `.ics`（`src/lib/ics.ts`）にして iOS の「カレンダーに追加（すべて追加）」を起動（**OAuth/サーバ不要**）。時刻は TZ 指定なしのフローティング＝端末ローカル(JST)。`UID=sh+uuid@yutori`、`SUMMARY=バイト`。**Googleカレンダーは使わない**（オーナーは iPhone 純正カレンダー利用のため。当初の Google OAuth 案＝`gcal.ts` は廃止）。TimeTree は「端末カレンダー表示」で純正カレンダーを映せる。※ワンショット追加なのでシフト変更時の再追加は重複の可能性あり（要実機確認）。将来：自動更新が要るなら webcal/.ics 購読（Edge Function でフィード配信・反映は数時間遅れ・読み取り専用）も選択肢。
...
- **スキャン画像PDFのデコードには pdf.js の WASM が必須（実害あり・対処済み）**: pdf.js **v6** は白黒スキャン画像（**JBIG2 / CCITTFax** 等の圧縮）のデコードを **WebAssembly** で行う。`getDocument` に **`wasmUrl`** を渡さないと画像がデコードできず、`renderPdfFirstPage` の canvas が**白紙**になり、複合機（例: RICOH IM C3510）スキャンの給与明細を画像化→Azure OCR しても **中身が空**（レビュー画面が全項目カラ・様式 azure-layout だけ表示）になる（警告 "Ensure that the `wasmUrl` API parameter is provided" / "Jbig2Error: JBig2 failed to initialize"）。CamScanner等の**写真スキャンは JPEG(DCTDecode)** でWASM不要のため従来動いていたが、**1bit CCITT/JBIG2 のFAX圧縮スキャンで発覚**。対処＝cMapと同じ方式：`vite.config.ts` の `pdfWasm`/`pdfWasmDev` で `node_modules/pdfjs-dist/wasm` を **dist/wasm** へコピー＆開発時 `/wasm/`（.wasmは`application/wasm`）で配信し、`extract.ts` が `wasmUrl = import.meta.env.BASE_URL + 'wasm/'` を全 `getDocument`（`PDF_OPTS`）に渡す。wasm(jbig2/openjpeg/qcms 約1.5MB) はリポジトリに含めずビルド時コピー。検証は `scratchpad` の repro（`@napi-rs/canvas`＋pdfjsで wasmUrl 有無を比較＝黒画素0%→1.62%で文字が出る）。
...
- **レイアウトはアプリシェル構造（実害あり・対処済み）**: `.app` を画面高（`100dvh`）で固定しスクロールさせず（`display:flex; flex-direction:column; overflow:hidden`）、スクロールするのは `.content` の内側だけ（`flex:1; overflow-y:auto; overscroll-behavior-y:contain`）。こうしないと iOS のページ全体スクロール（ゴムバンド／ツールバー連動）で**固定タブバー/FABが一緒に動いて見える**（オーナー報告で対処）。モーダルの背景固定は body の overflow ではなく **`body.modal-open .content{overflow:hidden}`**（スクロール領域が `.content` のため。ShiftCalendar が付与）。**重要：`.content` はiOSで独自の重なり順（stacking context）を作るため、その中で開くモーダルは固定タブバー/FABの下に潜って見えなくなる（実害あり・対処済み＝給与取込が下部に潜って見えない）。対策＝全モーダルの `.modal-backdrop` に `use:portal`（`src/lib/portal.ts`）を付け、body直下へ出して常に最前面にする**（新しいモーダルを追加したら必ず `use:portal` を付ける）。**上部バー（`.topbar`：yutori／サインアウト）は撤去**し、`.content` に `padding-top: env(safe-area-inset-top)` でノッチを回避。**サインアウトは設定タブ最下部の `.signout` ボタンに移動**。
