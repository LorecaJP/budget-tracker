...
└─ src/
   ...
   └─ components/
      ...
      ├─ Analysis.svelte
      ├─ Ledger.svelte          # 「収支」タブの親。上部トグル[月次｜年間]で下2つを切替（タブ id は 'year' のまま）
      ├─ MonthlyBreakdown.svelte# 月次の費目別 実績/予定（予算）。区分ごとにカード＋進捗バー＋予定超過の赤
...
- **タブナビ**（App.svelte）: ホーム / 取引 / 分析 / **収支** / **支払い** / 特別費 / **扶養** / 設定 の **8タブ**（「収支」タブの id は内部的に 'year'、「支払い」は 'pay'）。起動時に設定読込→シード→描画をゲート。
- **ホーム**（Home.svelte）: 今月（予算月）の収入・支出・収支、予算進捗バー、**貯金の目標カード（達成率バー）**、最近の取引（振替は中立表示）。
...
- **収支タブ＝予実ビュー**（Ledger.svelte / 上部トグル[月次｜年間]）: Excelの「各月シート」＋「収支表」の代替。**予算を『予定額』として使い、実績があれば実績・無ければ予定（薄字）で表示**（＝事前に予定を入れておき支出登録で置き換え、年間の見込みを可視化）。
  - **月次**（MonthlyBreakdown.svelte）: 予算月を切替、費目別に **実績／予定（予算）** を区分ごとのカードで表示。予定超過は赤＋進捗バー。サマリは 収入／支出／予定支出。
