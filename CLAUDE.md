...
└─ src/
   ...
   └─ components/
      ...
      ├─ FuyouTracker.svelte    # 扶養トラッカー（えみ）。暦年でえみ給料を集計し上限までの残り金額/時間。タブ「扶養」（独立タブのまま）。確定＋見込み(ShiftPlanner)で「あと働ける」を算出。**月別リストの空欄(未受給月)をタップ→時間(h/m)入力で見込みを手動登録**（×時給→`fuyou_overrides`、就労月=支給月-1。保存後 `{#key}` で ShiftPlanner を再マウントして再集計）。**月別リストはシフト由来の見込み（時間×時給）も自動表示**＝手動上書きが無い未受給月は `listShifts` から就労月(支給月-1)の総時間×時給で見込みを出す（`shiftHoursByPay`/`estFor`。「見込み（シフト ○h）」表示・タップで上書き編集も可）。集計は支給日ベースの暦年なので「今年あと」＝今年12月支給ぶんまで（12月の就労ぶんは翌1月支給＝翌年カウント）
