...
### パーサのポイント（実データで検証済み）
- 控除の内訳は **Azureの表(tables)のセル**から取る（ラベルの真下＝行+1・同列）。本文(content)はラベルと値が離れるため誤対応する。小計（社会保険料計）・課税対象額・控除合計は内訳に含めない。拾う控除は `parseAzure.ts` の `dedDefs`（健康保険/**子育支援金**/厚生年金/雇用保険/住民税/所得税）。**子育支援金＝2026年開始の「子ども・子育て支援金」**（健保と別枠で控除。実害あり・対処済み：これを拾わないと控除合計より368円等ぶん過少になり「差引支給額が一致しません」の誤警告が出る＝ゆうき2026-07明細で発生）。新しい控除項目が明細に増えたら `dedDefs` に追加する。seedの税カテゴリにも `子育支援金` を追加済み（既存DBは未作成なので、税区分に載せたい場合のみ設定でカテゴリ追加）。テキストPDF側(`parse.ts`＝えみ様式)は控除合計のみを1件で使うため内訳変更の影響なし。
