...
<div class="screen">
  ...
  {:else}
    ...
    <p class="hint">暦年（1〜12月）・支給日ベースで「えみ給料」を集計しています（年末調整還付は報酬ではないため対象外）。社保の106万判定は交通費・残業・賞与を除くので、交通費込みのこの表示より実際の余裕は少し多めです。</p>
    <p class="hint">勤務先が51人以上の場合、「週20時間以上 ＋ 月8.8万円以上」で社会保険の加入対象になります。年間だけでなく月8.8万ラインも超えないか確認してください。時給・上限は設定タブで変更できます。</p>
  {/if}
</div>
